...
<head>
    <meta charset="UTF-8">
    <title>Profile - Ayo Membaca</title>
...
<body class="bg-gray-100 text-black dark:bg-gray-900 dark:text-white">
    ...
    {{-- Navbar from home.blade.php --}}
@include('layouts.navigation')

    {{-- Add padding to avoid content being hidden by fixed navbar --}}
    <div class="h-32"></div>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            {{-- Combined Profile Sections into a single card --}}
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg space-y-6">

                {{-- Top row: Profile Information and Profile Photo --}}
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                    ...
                    <div class="md:col-span-1 justify-self-center">
                         {{-- Include only the Profile Photo part here --}}
                         @include('profile.partials.update-profile-photo-only') {{-- Temporarily include a new partial for just photo --}}
                    </div>
                </div>

                {{-- Separator below the top row --}}
                <div class="border-b border-gray-200 dark:border-gray-700"></div>

                {{-- Your Contributions Section (Full Width) --}}
                {{-- This section is now outside the top grid --}}
                <div> {{-- Add a div wrapper to give it space-y --}}
                    @include('profile.partials.your-contributions-only') {{-- Temporarily include a new partial for just contributions --}}
                </div>
